...
namespace Tests.PlayMode
{
    ...
    public class LearnerSessionIntegrationTests
    {
        ...
        [Test]
        [TestCase(true, false)]  // If newStatus is true, isFirstTimeUser should be false
        [TestCase(false, true)]  // If newStatus is false, isFirstTimeUser should be true
        [Category("BuildServer")]
        public void SetUserStatus_ShouldSetFirstTimeUserCorrectly(bool newStatus, bool expectedIsFirstTimeUser)
        {
            // Act
            learnerSession.SetUserStatus(newStatus);

            // Assert
            Assert.AreEqual(expectedIsFirstTimeUser, learnerSession.IsFirstTimeUser);
        }
    }
